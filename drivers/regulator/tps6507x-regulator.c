...
static int tps6507x_ldo_get_voltage(struct regulator_dev *dev)
{
	...
	if (ldo < TPS6507X_LDO_1 || ldo > TPS6507X_LDO_2)
		...
		reg = (ldo == TPS6507X_LDO_1 ? TPS6507X_REG_LDO_CTRL1 :
										TPS6507X_REG_DEFLDO2);
		mask = (ldo == TPS6507X_LDO_1 ? TPS6507X_REG_LDO_CTRL1_LDO1_MASK :
...
int tps_6507x_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	for (i = 0; i < TPS6507X_NUM_REGULATOR; i++, info++, init_data++) {
		...
		rdev = regulator_register(&tps->desc[i], &client->dev, init_data, tps);
		if (IS_ERR(rdev)) {
