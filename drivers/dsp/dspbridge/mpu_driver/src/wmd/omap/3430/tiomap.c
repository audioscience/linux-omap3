...
static DSP_STATUS WMD_BRD_Monitor(struct WMD_DEV_CONTEXT *hDevContext)
{
    ...
    if ((temp & 0x03) != 0x03 || (temp & 0x03) != 0x02) {
		...
		HAL_PWR_IVA2StateGet(resources.dwPrmBase, HAL_PWR_DOMAIN_DSP,
				     &pwrState);
		HAL_PWR_IVA2PowerStateSet(resources.dwPrmBase,
					  HAL_PWR_DOMAIN_DSP,
					  HAL_PWR_STATE_ON);
		/* Set the SW supervised state transition */
		HAL_PWR_CLKCTRL_IVA2RegSet(resources.dwCmBase,
					   HAL_SW_SUP_WAKEUP);
				...
				DWORD dwDSPAddr)
{
	...
	* from ulTLBBaseVirt */
	...
	if (DSP_SUCCEEDED(status)) {
		...
		if (dsp_debug) {
			...
			DBG_Trace(DBG_LEVEL7,
				 "Set boot mode to self loop for IVA2 "
				 "Device\n");
			...
			DBG_Trace(DBG_LEVEL7,
				 "Set boot mode to direct boot for IVA2 "
				 "Device \n");
			HAL_DSPSS_BootModeSet(resources.dwSysCtrlBase,
	...
	temp = (UWORD32) *((REG_UWORD32 *) ((UWORD32) (resources.dwDmmuBase) +
		0x10));
	...
	*((REG_UWORD32 *) ((UWORD32) (resources.dwDmmuBase) + 0x10)) =
	...
	if (ulLoadMonitorTimer != 0xFFFF) {
		...
		if (!DSP_SUCCEEDED(status)) {
			DBG_Trace(DBG_LEVEL7,
				 " Error while setting LM Timer  to 32KHz\n");
		}
	   ...
	   if (ulBiosGpTimer != 0xFFFF) {
		uClkCmd = (BPWR_DisableClock << MBX_PM_CLK_CMDSHIFT) |
			  ulBiosGpTimer;
		DBG_Trace(DBG_LEVEL7,
			  "encoded BIOS GPTimer cmd for Disable: 0x%x\n",
			  uClkCmd);
		DSPPeripheralClkCtrl(pDevContext, &uClkCmd);
	...
	if (DSP_SUCCEEDED(status)) {
		...
		HAL_MMU_EventEnable(resources.dwDmmuBase,
				    HAL_MMU_ALL_INTERRUPTS);
	...
	/* Write the synchronization bit to indicate the completion of OPP table
	    update to DSP
	 */
				 ...
				 IN CONST struct CFG_DSPRES *pDspConfig)
{
	...
	ULONG   pg_tbl_va;
	ULONG   align_size;
	ULONG numOfPhyAddrTableEntries;
	...
	numOfPhyAddrTableEntries = DMMPOOLSIZE/HAL_PAGE_SIZE_4KB;
	pPhysAddrPageTbl = NULL;
	/* This table will be used to store the virtual to physical
	 * address translations */
	pPhysAddrPageTbl = (ULONG *)MEM_Calloc(numOfPhyAddrTableEntries *
			   sizeof(ULONG), MEM_NONPAGED);
	if (pPhysAddrPageTbl == NULL) {
		status = DSP_EMEMORY;
		DBG_Trace(DBG_LEVEL7, "WMD_DEV_Create: Memory allocation for "
			  "pPhysAddrPageTbl failed\n");
		goto func_end;
	}
	DBG_Trace(DBG_LEVEL4, "WMD_DEV_Create: Allocated memory for"
		  "numOfPhyAddrTableEntries= %d entries \n",
		  numOfPhyAddrTableEntries);
				 ...
				 ULONG ulNumBytes, ULONG ulMapAttr)
{
	...
	while (temp < numOfActualTabEntries) {
		status = PteSet(pDevContext->pPtAttrs, pPhysAddrPageTbl[temp++],
				ulVirtAddr, HAL_PAGE_SIZE_4KB, &halAttrs);
		if (DSP_FAILED(status)) {
			DBG_Trace(DBG_LEVEL7,
				 "WMD_BRD_MemMap: FAILED IN PTESET \n");
			return DSP_EFAIL;
		}
		ulVirtAddr += HAL_PAGE_SIZE_4KB;
	}
	if (DSP_FAILED(status))
				   ...
				   ULONG ulVirtAddr, ULONG ulNumBytes)
{
	...
	while (remBytes && (DSP_SUCCEEDED(status))) {
		...
		if (pteSize == HAL_MMU_COARSE_PAGE_SIZE) {
			...
			pteCount = (HAL_MMU_COARSE_PAGE_SIZE - pteCount) /
				    sizeof(ULONG);
			...
			while (remBytesL2 && (DSP_SUCCEEDED(status))) {
				...
				if ((pteSize != 0) && (remBytesL2 >= pteSize) &&
				   !(vaCurr & (pteSize - 1))) {
					HAL_MMU_PteClear(pteAddrL2, vaCurr,
							 pteSize);
					remBytesL2 -= pteSize;
					vaCurr += pteSize;
			...
			if (remBytesL2 == 0) {
				...
				if (pt->pgInfo[L2PageNum].numEntries == 0) {
					 ...
					 * L2 PT */
					status = HAL_MMU_PteClear(L1BaseVa,
						 vaCurrOrig,
						 HAL_MMU_COARSE_PAGE_SIZE);
				}
					...
					ULONG *physicalAddrTable)
{
	...
	down_read(&mm->mmap_sem);
	/* This call invokes handle_mmu _fault call, which causes all pages to
	 * be created before we scan the page tables */
	numUsrPgs = get_user_pages(curr_task, mm, ulMpuAddr, ulNumBytes, TRUE,
				  0, NULL, NULL);
	up_read(&mm->mmap_sem);
			       ...
			       UWORD32 sysctrl_base)
{
	...
	HAL_PWRST_IVA2RegGet(prm_base, &temp);
	 if ((temp & 0x03) != 0x03 || (temp & 0x03) != 0x02) {
		...
		HAL_PWR_IVA2PowerStateSet(prm_base, HAL_PWR_DOMAIN_DSP,
					  HAL_PWR_STATE_ON);
		/* Set the SW supervised state transition */
