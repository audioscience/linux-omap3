/*
 * dspbridge/src/wmd/linux/omap/common/mmu_fault.c
 *
...
VOID MMU_FaultIsr(IN PVOID pRefData)
{
	...
	if (MEM_IsValidHandle(pDehMgr, SIGNATURE)) {
		...
		if (MMU_CheckIfFault(pDevContext)) {
			...
			/* Disable the MMU events, else once we clear it will
			 * start to raise INTs again */
			HAL_MMU_EventDisable(resources.dwDmmuBase,
...
static BOOL MMU_CheckIfFault(struct WMD_DEV_CONTEXT *pDevContext)
{
	...
	halStatus = HAL_MMU_EventStatus(resources.dwDmmuBase, &dmmuEventMask);
	if (dmmuEventMask  ==  HAL_MMU_TRANSLATION_FAULT) {
