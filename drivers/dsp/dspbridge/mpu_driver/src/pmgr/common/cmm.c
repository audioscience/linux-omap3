/*
 * dspbridge/src/pmgr/linux/common/cmm.c
 *
...
struct CMM_ALLOCATOR {	/* sma */
	DWORD dwSignature;	/* SMA allocator signature SMEMSIGNATURE */
	DWORD dwSmBase;		/* Start of physical SM block */
	ULONG ulSmSize;		/* Size of SM block in bytes */
	DWORD dwVmBase;		/* Start of VM block. (Dev driver context
				 * for 'sma') */
	DWORD dwDSPPhysAddrOffset;	/* DSP PA to GPP PA offset for this
					 * SM space */
	CMM_CNVTTYPE cFactor;	/* CMM_ADDTO[SUBFROM]DSPPA, _POMAPEMIF2DSPBUS */
	DWORD dwDSPBase;	/* DSP virt base byte address */
...
struct CMM_XLATOR {	/* Pa<->Va translator object */
	 ...
	 */
	DWORD dwVirtBase;	/* virtual base address */
	ULONG ulVirtSize;	/* size of virt space in bytes */
	ULONG ulSegId;		/* Segment Id */
} ;
...
struct CMM_OBJECT {
	...
	ULONG ulMinBlockSize;	/* Min SM block; default 16 bytes */
	DWORD dwPageSize;	/* Memory Page size (1k/4k) */
	/* GPP SM segment ptrs */
...
struct CMM_MNODE {
	...
	DWORD dwPA;		/* Phys addr */
	DWORD dwVA;		/* Virtual address in device process context */
	ULONG ulSize;		/* SM block size in bytes */
...
static struct CMM_MNODE *GetFreeBlock(struct CMM_ALLOCATOR *pAllocator,
				      UINT uSize);
static struct CMM_MNODE *GetNode(struct CMM_OBJECT *pCmmMgr, DWORD dwPA,
				 DWORD dwVA, ULONG ulSize);
/* get available slot for new allocator */
static INT GetSlot(struct CMM_OBJECT *hCmmMgr);
 ...
 *      inUseList.
 */
PVOID CMM_CallocBuf(struct CMM_OBJECT *hCmmMgr, UINT uSize,
		    struct CMM_ATTRS *pAttrs, OUT PVOID *ppBufVA)
{
	...
	if ((MEM_IsValidHandle(pCmmMgr, CMMSIGNATURE)) && (uSize != 0)) {
		if (pAttrs->ulSegId > 0) {	/* SegId > 0 is SM  */
		...
		if (pNode) {
			if ((uDeltaSize = (pNode->ulSize - uSize)) >=
			    pCmmMgr->ulMinBlockSize) {
			 ...
			 * client process goes away. See CMM_ClientProcDetach()
			 */
			(Void)PRCS_GetCurrentHandle(&pNode->hClientProc);
			/* put our node on InUse list */
			LST_PutTail(pAllocator->pInUseListHead,
				   (struct LST_ELEM *)pNode);
			pBufPA = (PVOID)pNode->dwPA;	/* physical address */
			/* clear mem */
			pByte = (BYTE *)pNode->dwVA;
			for (cnt = 0; cnt < (INT) uSize; cnt++, pByte++)
				*pByte = 0;

			if (ppBufVA != NULL) {
				/* Virtual address */
				*ppBufVA = (PVOID)pNode->dwVA;
			}
		}
	...
	*phCmmMgr = NULL;
	...
	if (pCmmObject != NULL) {
...
#ifndef LINUX
		status = UTIL_GetSysInfo(&sysInfo);
#else
		...
		if (DSP_SUCCEEDED(status)) {
			...
			if ((pCmmObject->pNodeFreeListHead = LST_Create()) ==
			     NULL) {
				GT_0trace(CMM_debugMask, GT_7CLASS,
...
DSP_STATUS CMM_Destroy(struct CMM_OBJECT *hCmmMgr, BOOL bForce)
{
	...
	if (!bForce) {
		...
		if (DSP_SUCCEEDED(status = CMM_GetInfo(hCmmMgr, &tempInfo))) {
 ...
 */
DSP_STATUS CMM_FreeBuf(struct CMM_OBJECT *hCmmMgr, PVOID pBufPA, ULONG ulSegId)
{
	struct CMM_OBJECT *pCmmMgr = (struct CMM_OBJECT *)hCmmMgr;
	...
	if ((pAllocator = GetAllocator(pCmmMgr, ulSegId)) != NULL) {
		SYNC_EnterCS(pCmmMgr->hCmmLock);
		pCurNode = (struct CMM_MNODE *)LST_First(pAllocator->
			    pInUseListHead);
		while (pCurNode) {
			if ((DWORD)pBufPA == pCurNode->dwPA) {
				/* Found it */
		       ...
		       OUT struct CMM_INFO *pCmmInfo)
{
	...
	for (ulSeg = 1; ulSeg <= CMM_MAXGPPSEGS; ulSeg++) {
		/* get the allocator object for this segment id */
		if ((pAltr = GetAllocator(pCmmMgr, ulSeg)) != NULL) {
 ...
 *  ======== CMM_ClientProcDetach ========
 *  Purpose:
 *      Perform cmm mgr cleanup for this detached process.
 */
DSP_STATUS CMM_ClientProcDetach(struct CMM_OBJECT *hCmmMgr, HANDLE hProcess)
{
	struct CMM_OBJECT *pCmmMgr = (struct CMM_OBJECT *)hCmmMgr;
	DSP_STATUS status = DSP_SOK;
	struct CMM_MNODE *pCurNode = NULL;
	struct CMM_ALLOCATOR *pAltr = NULL;
	ULONG ulSeg;
	...
	/* Scan all SM memory segments for memory nodes that
	 *   were allocated by the client hProcess.  */
	SYNC_EnterCS(pCmmMgr->hCmmLock);
	for (ulSeg = 1; ulSeg <= CMM_MAXGPPSEGS; ulSeg++) {
		/* get the allocator object for this segment id */
		if ((pAltr = GetAllocator(pCmmMgr, ulSeg)) == NULL)
		...
		pCurNode = (struct CMM_MNODE *)LST_First(pAltr->pInUseListHead);
		while (pCurNode) {
			if (pCurNode->hClientProc == hProcess) {
				/* this process allocated this node */
				LST_RemoveElem(pAltr->pInUseListHead,
					      (struct LST_ELEM *)pCurNode);
				/* back to freelist */
				AddToFreeList(pAltr, pCurNode);
			}
			/* next node. */
			pCurNode = (struct CMM_MNODE *)LST_Next(pAltr->
				   pInUseListHead,
	...
	}		/* end for all registered GPP SM segments */
	SYNC_LeaveCS(pCmmMgr->hCmmLock);
	return (status);
}
 ...
 *      allocations.
 */
DSP_STATUS CMM_RegisterGPPSMSeg(struct CMM_OBJECT *hCmmMgr, DWORD dwGPPBasePA,
				ULONG ulSize, DWORD dwDSPAddrOffset,
				CMM_CNVTTYPE cFactor, DWORD dwDSPBase,
				ULONG ulDSPSize, ULONG *pulSegId,
				DWORD dwGPPBaseVA)
{
	struct CMM_OBJECT *pCmmMgr = (struct CMM_OBJECT *)hCmmMgr;
	...
	SYNC_EnterCS(pCmmMgr->hCmmLock);
	if ((nSlot = GetSlot(pCmmMgr)) < 0) {	/* get a slot number */
		status = DSP_EFAIL;
	}
	...
	if (pSMA != NULL) {
		...
		if (dwGPPBasePA != 0) {
			pSMA->dwVmBase =
				(DWORD)MEM_LinearAddress((PVOID)dwGPPBasePA,
				ulSize);
		}
#else
		...
		if (DSP_SUCCEEDED(status)) {
			...
			*pulSegId = (ULONG) nSlot + 1;
			/* create memory free list */
			if ((pSMA->pFreeListHead =
			     LST_Create()) == NULL) {
				GT_0trace(CMM_debugMask, GT_7CLASS,
					  "CMM_RegisterGPPSMSeg: "
					  "Out Of Memory \n");
		...
		if (DSP_SUCCEEDED(status)) {
			...
			if ((pSMA->pInUseListHead = LST_Create()) == NULL) {
				GT_0trace(CMM_debugMask, GT_7CLASS,
					  "CMM_RegisterGPPSMSeg: "
					  "LST_Create failed\n");
				status = DSP_EMEMORY;
 ...
 */
DSP_STATUS CMM_UnRegisterGPPSMSeg(struct CMM_OBJECT *hCmmMgr, ULONG ulSegId)
{
	struct CMM_OBJECT *pCmmMgr = (struct CMM_OBJECT *)hCmmMgr;
	DSP_STATUS status = DSP_SOK;
...
static VOID UnRegisterGPPSMSeg(struct CMM_ALLOCATOR *pSMA)
{
	...
	if ((PVOID) pSMA->dwVmBase != NULL)
		MEM_UnmapLinearAddress((PVOID) pSMA->dwVmBase);
 ...
 */
static INT GetSlot(struct CMM_OBJECT *pCmmMgr)
{
	INT nSlot = -1;		/* neg on failure */
					  ...
					  ULONG ulSegId)
{
	...
	if ((pAllocator = pCmmMgr->paGPPSMSegTab[ulSegId - 1]) != NULL) {
 ...
 *  ======== CMM_XlatorAllocBuf ========
 */
PVOID CMM_XlatorAllocBuf(struct CMM_XLATOROBJECT *hXlator, PVOID pVaBuf,
			UINT uPaSize)
{
	struct CMM_XLATOR *pXlator = (struct CMM_XLATOR *)hXlator;
	PVOID pBuf = NULL;
	...
	if (MEM_IsValidHandle(pXlator, CMMXLATESIGNATURE)) {
		...
		*(volatile DWORD *)pVaBuf = 0;
		/* Alloc SM */
		if ((pBuf = CMM_CallocBuf(pXlator->hCmmMgr, uPaSize, &attrs,
		    NULL))) {
			/* convert to translator(node/strm) process Virtual
			 * address */
			*(volatile DWORD **)pVaBuf =
				 (DWORD *)CMM_XlatorTranslate(hXlator,
							      pBuf, CMM_PA2VA);
		}
	}
	return (pBuf);
 ...
 */
DSP_STATUS CMM_XlatorFreeBuf(struct CMM_XLATOROBJECT *hXlator, PVOID pBufVa)
{
	struct CMM_XLATOR *pXlator = (struct CMM_XLATOR *)hXlator;
	...
	if (MEM_IsValidHandle(pXlator, CMMXLATESIGNATURE)) {
		...
		if ((pBufPa = CMM_XlatorTranslate(hXlator, pBufVa,
						  CMM_VA2PA))) {
 ...
 */
DSP_STATUS CMM_XlatorInfo(struct CMM_XLATOROBJECT *hXlator, IN OUT BYTE **pAddr,
			 ULONG ulSize, UINT uSegId, BOOL bSetInfo)
{
	struct CMM_XLATOR *pXlator = (struct CMM_XLATOR *)hXlator;
	DSP_STATUS status = DSP_SOK;
	...
	if (MEM_IsValidHandle(pXlator, CMMXLATESIGNATURE)) {
		if (bSetInfo) {
			...
			pXlator->dwVirtBase = (DWORD)*pAddr;
 ...
 *  ======== CMM_XlatorTranslate ========
 */
PVOID CMM_XlatorTranslate(struct CMM_XLATOROBJECT *hXlator, PVOID pAddr,
			  CMM_XLATETYPE xType)
{
	DWORD dwAddrXlate = 0;
	struct CMM_XLATOR *pXlator = (struct CMM_XLATOR *)hXlator;
	...
	if ((xType == CMM_VA2DSPPA) || (xType == CMM_VA2PA) ||
		...
		if (xType == CMM_PA2VA) {
			...
			dwOffset = (BYTE *)pAddr - (BYTE *)(pAlctr->dwSmBase -
				    pAlctr->ulDSPSize);
			...
			dwOffset = (BYTE *)pAddr - (BYTE *)pXlator->dwVirtBase;
			dwAddrXlate = pAlctr->dwSmBase - pAlctr->ulDSPSize +
				      dwOffset;
		}
	} else {
		dwAddrXlate = (DWORD)pAddr;
	}
 ...
 *      Used internal by Bridge.
 *      Assumes buffer allocated by CMM.
 */
PVOID CMM_KernConvert(struct CMM_OBJECT *hCmmMgr, PVOID pAddr,
		     struct CMM_ATTRS *pAttrs, CMM_KERNMAPTYPE xType)
{
	struct CMM_OBJECT *pCmmMgr = (struct CMM_OBJECT *)hCmmMgr;
	PVOID pNewAddr = NULL;
	ULONG ulSegId;
	struct CMM_ALLOCATOR *pAlctr;

	DBC_Assert(pAddr != NULL);
	DBC_Assert((xType == CMM_KERNVA2DSP) || (xType == CMM_DSP2KERNVA));
	if (pAttrs == NULL)
		pAttrs = &CMM_DFLTALCTATTRS;

	ulSegId = pAttrs->ulSegId;	/* need segment Id */
	pAlctr = pCmmMgr->paGPPSMSegTab[ulSegId - 1];	/* get SM allocator */
	if (MEM_IsValidHandle(pAlctr, SMEMSIGNATURE)) {
		if (xType == CMM_KERNVA2DSP) {
			/* range check input address */
			if (((DWORD)pAddr < pAlctr->dwVmBase) ||
			    ((DWORD)pAddr >= (pAlctr->dwVmBase +
			    pAlctr->ulSmSize))) {
				pNewAddr = NULL;
			} else {
				/* convert to dsp address
				 * [DSPBase+DSPSize + (Delta Vm)] */
				/* (DWORD) pNewAddr = (pAlctr->dwDSPBase +
				 * pAlctr->ulDSPSize)
				 * +((DWORD) pAddr - pAlctr->dwVmBase);*/
				pNewAddr = (PVOID)((pAlctr->dwDSPBase +
					   pAlctr->ulDSPSize) +
					   ((DWORD) pAddr - pAlctr->dwVmBase));
			}
		} else {	/* CMM_DSP2KERNVA */
			DBC_Assert(FALSE);	/* not currently supported */
			pNewAddr = NULL;
		}
	} else {
		GT_0trace(CMM_debugMask, GT_6CLASS,
			  "CMM_KernConvert: ERROR - Bad SM "
			  "allocator/segment\n");
	}
	if (pNewAddr == NULL) {
		GT_0trace(CMM_debugMask, GT_6CLASS,
			  "CMM_KernConvert: DSP_FAILED to "
			  "convert address\n");
		DBC_Assert(FALSE);
	} else {
		GT_3trace(CMM_debugMask, GT_3CLASS,
			  "CMM_KernConvert pAddr %x xType %x"
			  "pNewAddr %x\n", pAddr, xType, pNewAddr);
	}
	return (pNewAddr);
}
