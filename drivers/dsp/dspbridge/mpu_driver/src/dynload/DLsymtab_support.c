/*
 * dspbridge/src/dynload/DLsymtab_support.c
 *
 ...
 *
 ************************************************************************/
struct symbol *Find_Matching_Symbol(symtab *a_stable, const char *name)
{
	struct symbol *sptr;
	char c;
	...
	a_stable->hash_key = (uint32_t)(key * AVALUE) >>
			     (int)*a_stable->mytable;
				 ...
				 const char *nname, unsigned versn)
{
	...
	sptr = &a_stable->mytable[a_stable->hash_key + 1];
	newsym->link = (struct ssymbol *) *sptr;
	*sptr = newsym;
...
void Iterate_Symbols(struct symbol **stable, Symbol_Action *action, void *arg)
{
	...
	cnt = (~UINT32_C(0) >> (unsigned) *stable++) + 1;
	/* length of hash table */
	do {
