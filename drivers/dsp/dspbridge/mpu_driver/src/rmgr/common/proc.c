		    ...
		    IN CONST CHAR **aArgv, IN CONST CHAR **aEnvp)
{
	...
	if (uProcId == DSP_UNIT) {
		 /* Use all available DSP address space after EXTMEM for DMM */
		if (DSP_SUCCEEDED(status)) {
			status = COD_GetSymValue(hCodMgr, EXTEND, &dwExtEnd);
			if (DSP_FAILED(status)) {
				GT_1trace(PROC_DebugMask, GT_7CLASS,
					 "PROC_Load: Failed on "
					 "COD_GetSymValue %s.\n", EXTEND);
		...
		if (DSP_SUCCEEDED(status)) {
			...
			if (DSP_SUCCEEDED(status)) {
				DMM_Reset(hDmmMgr);
				/* Set dwExtEnd to DMM START BYTE address */
				dwExtEnd = (dwExtEnd + 1) * DSPWORDSIZE;
				 /* DMM memory is from EXT_END to start
				  *  of IOMA=0x3F the end address for DMM is
				  *  set to the beginning of the CMDWR and L4
...
DSP_STATUS PROC_UnReserveMemory(DSP_HPROCESSOR hProcessor, PVOID pRsvAddr)
{
	...
	/*if (DSP_FAILED(status)) {
		...
		ULONG mapCount;
		ULONG maxCount = 16;
		ULONG mapList[maxCount];
		ULONG status1 = DSP_SOK;

		mapCount = maxCount;

		while (mapCount && (DSP_SUCCEEDED(status1))) {
			status1 =
			    DMM_BlockList(hDmmMgr, (ULONG) pRsvAddr, &mapCount,
					  mapList);

			for (i = 0; (i < mapCount) && DSP_SUCCEEDED(status1);
			     i++) {
				status1 =
				    PROC_UnMap(hProcessor, (PVOID) mapList[i]);
			}
		}

		if (DSP_SUCCEEDED(status1)) {*/
			/* Now that all blocks are unmapped, try again.
			status = DMM_UnReserveMemory(hDmmMgr, (ULONG) pRsvAddr);
		}
	}*/
#else
	if (DSP_FAILED(status)) {
		...
		mapCount = maxCount;
		while (mapCount && (DSP_SUCCEEDED(status1))) {
			status1 = DMM_BlockList(hDmmMgr, (ULONG)pRsvAddr,
						&mapCount, mapList);
			for (i = 0; (i < mapCount) && DSP_SUCCEEDED(status1);
			    i++) {
				status1 = PROC_UnMap(hProcessor,
					 (PVOID) mapList[i]);
			}
		}
		if (DSP_SUCCEEDED(status1)) {
