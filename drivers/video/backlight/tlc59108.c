...
struct i2c_client *tlc_client, *pcf_client;
...
static int enable_lcd_power(struct i2c_client *c, int enable)
{
	int ret;
	...
	if (ret < 0) {
		printk(KERN_ERR "I2C read failed for PCF %d\n", ret);
		return ret;
	}
	if (enable)
		buf[0] &= ~(1 << 0);
	...
	ret = i2c_transfer(c->adapter, &msg, 1);
	if (ret < 0) {
		printk(KERN_ERR "I2C write failed for PCF\n");
		return ret;
	}
	pcf_client = c;
	return 0;
...
static int tlc59108_probe(struct i2c_client *c, const struct i2c_device_id *id)
{
	...
	if (id->driver_data == 0) {
		...
		data->bl = backlight_device_register("tlc59108-bl",
						&c->dev, data,
						&bl_ops, &props);
		if (IS_ERR(data->bl)) {
			ret = PTR_ERR(data->bl);
...
{
	struct tlc59108_bl *data;


	if (c == tlc_client) {
		data = i2c_get_clientdata(c);
		backlight_device_unregister(data->bl);
		data->bl = NULL;
		kfree(data);
		tlc_client = NULL;
	} else {
		enable_lcd_power(c, 0);
	}
	return 0;
}
