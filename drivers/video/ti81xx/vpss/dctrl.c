...
{
	struct vps_dcoutputinfo *oinfo = &binfo->outinfo;
	int r = 0;
	int l = 0, i;
				     ...
				     size_t size)
{
	...
	if (!r) {
		r = size;
		memcpy(&binfo->outinfo, &oinfo, sizeof(oinfo));
	}
exit:
	dc_unlock(binfo->dctrl);
	return r;
			  ...
			  const char *clksrc)
{
	...
	for (i = 0; i < disp_ctrl->numvencs; i++) {
		r = dc_set_output(&disp_ctrl->blenders[i].outinfo);
		if (r) {
			VPSSERR("failed to set venc output\n");
			goto cleanup;
		}
