/*
 * drivers/video/omap_fb.c
 *
 * Framebuffer driver for OMAP display controller.
 *
...
#define FB_NAME 	"omapfb"	/* 16 chars max */

#define SCHEDULE_WAIT	0
#define BUSY_WAIT	1

/* To use the rotation feature, include this in your boot params:
	video=omapfb:rotation=[0|90|180|270]
*/

int omapfb_rotation = -1;	// -1 = no rotation support
int omapfb_mirroring = 0;	// the status of mirroring
...
static struct omapfb_info *saved_oinfo;
static struct fb_var_screeninfo default_var;

int	fb_out_layer = OMAP_GRAPHICS;
...
/* omapfb_gfx_clk_period() must return the period (in picoseconds) of the
 * graphics timebase clock.  This clock is the input to the pixel clock
 * divider.
 *
 * prototype:
 * unsigned long omapfb_gfx_clk_period(void);
 */
#define omapfb_gfx_clk_period() (1000000000UL/(omap_get_dss1_clock()/1000))

/* omapfb_fb_base() must return the physical base address of the
 ...
 *
 * prototype:
 * unsigned long omapfb_fb_base(void);
 */
#define omapfb_fb_base() 0

/* omapfb_fb_size() must return the size of the framebuffer in bytes.
 ...
 * prototype:
 * unsigned long omapfb_fb_size(void);
 */
#define omapfb_fb_size(rotation) \
	rotation ? (2048 * 720 * (32/8)) : (480 * 720 * (32/8) * 2)
/* 720 x 576 is the max framebuffer size we allow for TV (PAL) */

/* omapfb_vrfb_size() must return the size of the virtual rotated
 * framebuffer.
 *
 * prototype:
 * unsigned long omapfb_fb_size(void);
 */
#define omapfb_vrfb_size()	(MAX_PIXELS_PER_LINE * 720 * (32/8))

/* omap_display_pixclock_max() must return the maximum pixclock period
 * supported by the display.
 *
 * prototype:
 * unsigned int omap_display_pixclock_max(void);
 */
#define omap_display_pixclock_max(ouput_dev) \
		(get_panel_pixclock_max(output_dev))

/* omap_display_pixclock_min() must return the minimum pixclock period
 * supported by the display.
 *
 * prototype:
 * unsigned int omap_display_pixclock_min(void);
 */
#define omap_display_pixclock_min(output_dev) \
		(get_panel_pixclock_min(output_dev))

/* omapfb_default_var() must return a pointer to a default
 * fb_var_screeninfo struct that will be used to set the initial video mode.
 * If this video mode is invalid (as determined by omapfb_check_var) then
 * the driver will fail to initialize.
 *
 * prototype:
 * struct fb_var_screeninfo *omapfb_default_var(void);
 */

static struct fb_var_screeninfo *
omapfb_default_var(void)
{
	struct fb_var_screeninfo *v = &default_var;
	int output_dev = omap_disp_get_output_dev(OMAP_GRAPHICS);
	u32 tmp;

	get_panel_default_var(v, output_dev);

	if (omapfb_rotation >= 0) {
		v->xres_virtual = v->yres_virtual = max(v->xres, v->yres);

		switch(omapfb_rotation) {
			case 0:
			default:
				v->xoffset	= 0;
				v->yoffset	= 0;
...
/* omapfb_check_mode() must check the video mode specified in a
 * fb_var_screeninfo struct and return 0 if the mode is supported and non-zero
 * if the mode is not supported.  omapfb_check_mode() is permitted to
 * modify the var to make it conform to a supported mode.
 *
 * prototype:
 * int omapfb_check_mode(const struct omapfb_info *oinfo,
 *			     struct fb_var_screeninfo *var);
 */
static int
omapfb_check_mode(const struct omapfb_info *oinfo,
		      struct fb_var_screeninfo *var)
{
	...
	omap_disp_get_dss();
	output_dev = omap_disp_get_output_dev(OMAP_GRAPHICS);

	omap_disp_get_panel_size(output_dev, &display_xres, &display_yres);
	...
	if(saved_oinfo->info.var.pixclock != var->pixclock) {
		omap_disp_put_dss();
		return -EINVAL;
	}
...
#ifdef CONFIG_PM
struct omapfb_suspend_data {
	/* Power management suspend lockout stuff */
	int suspended;
	wait_queue_head_t suspend_wq;
};
static struct omapfb_suspend_data fb_suspend_data;

#define omapfb_suspend_lockout_fp(s,f) \
	if ((s)->suspended) {\
...
#define omapfb_suspend_lockout(s) \
	if ((s)->suspended) {\
		wait_event_interruptible((s)->suspend_wq,\
					 (s)->suspended == 0);\
	}
#else
#define omapfb_suspend_lockout_fp(s, f) do {} while(0)
#define omapfb_suspend_lockout(s) do {} while(0)
#endif	/*ifdef CONFIG_PM */

/******************************************************************************/

/* Bits-per-pixel and color depth aren't quite the same thing.  The OMAP
...
horizontal_sync_freq(const struct fb_var_screeninfo *var)
{
	...
	omap_disp_pixels_per_clock(&nom, &den);

	/* get the horizontal display resolution */
	omap_disp_get_panel_size(output_dev, &xres, &yres);
...
static void
omapfb_isr(void *arg, struct pt_regs *regs, u32 irqstatus)
{
	struct omapfb_info *oinfo = (struct omapfb_info *) arg;
 ...
 */
static int
omapfb_wait_for_vsync(struct omapfb_info *oinfo)
{
	wait_queue_t wqt;
	unsigned long cnt;
	int ret;
	...
	omap_disp_irqenable(omapfb_isr,mask);
	...
	omap_disp_irqdisable(omapfb_isr,~(mask));
...
#ifdef DEBUG
static void
omapfb_synclost_isr(void *arg, struct pt_regs *regs)
{
	printk("<sl digital>");
}
#endif
...
wait_for_reg_sync(int busy_wait, unsigned long timeout)
{
	...
	while (!omap_disp_reg_sync_done(output_dev) &&
	...
	if (!omap_disp_reg_sync_done(output_dev)) {
...
/*
 *	omapfb_check_var - Validates a var passed in.
 ...
 */
static int
omapfb_check_var(struct fb_var_screeninfo *var, struct fb_info *info)
{
	const struct omapfb_info *oinfo =
		(const struct omapfb_info *) info->par;
...
static int
omapfb_set_dma_params(const struct fb_var_screeninfo *var,
			  ...
			  int rotation, int mirroring)
{
	...
	if (!mirroring) {
		...
		omap_disp_set_dma_params(OMAP_GRAPHICS, OMAP_OUTPUT_LCD,
				start, start, row_inc, pix_inc);

		omap_disp_set_dma_params(OMAP_GRAPHICS, OMAP_OUTPUT_TV,
		...
		if (rotation < 0) {
			...
			omap_disp_set_dma_params(OMAP_GRAPHICS,
					OMAP_OUTPUT_LCD,
					start, start,
					row_inc,
					pix_inc);
...
/*
 *	omapfb_set_par - Alters the hardware state.
 *	@info: frame buffer structure that represents a single frame buffer
 *
 ...
 *	omapfb_check_var is always called before omapfb_set_par to
 *	ensure this.
 *
 */

static int
omapfb_set_par(struct fb_info *info)
{
	struct omapfb_info *oinfo = (struct omapfb_info *) info->par;
	...
	if ((oinfo->rotation_support) &&
		...
		omap_disp_config_gfxlayer(var->yres, var->xres,
				var_to_depth(var));
	} else {
		omap_disp_config_gfxlayer(var->xres, var->yres,
				var_to_depth(var));
	}

	omap_disp_set_gfx_palette(oinfo->palette_phys);
	...
	if (oinfo->rotation_support) {
		...
		omap_disp_set_vrfb(0, oinfo->fb_base_phys,
	...
	ret = omapfb_set_dma_params(var, &info->fix,
	...
	omap_disp_config_lcd(clkdiv, hbp, hfp, hsw, vbp, vfp, vsw);

	if (!ret) {
		if(fb_out_layer == OMAP_GRAPHICS)
			omap_disp_start_gfxlayer();
		else{
...
int
omapfb_set_output_layer(int layer)
{
	struct omapfb_info *oinfo = (struct omapfb_info *) saved_oinfo;
	int ret = 0;
	...
	if(layer == OMAP_GRAPHICS){
#ifdef DEBUG
		printk("Switching FB to GFX pipeline \n");
#endif
		omapfb_set_par(&saved_oinfo->info);
	} else {
#ifdef DEBUG
		...
		omap_disp_config_vlayer(layer,&pix,&crop,&win,rotate,omapfb_mirroring);
		mdelay(1);
		omap_disp_start_vlayer(layer,&pix,&crop,&win, (rotate>=0)? SMS_ROT_VIRT_BASE(0,0):oinfo->info.fix.smem_start,rotate,omapfb_mirroring);
		wait_for_reg_sync(SCHEDULE_WAIT, oinfo->timeout);
	}
...
/*
 *  	omapfb_setcolreg - Sets a color register.
 ...
 */
static int
omapfb_setcolreg(unsigned regno, unsigned red, unsigned green,
...
/*
 *	omapfb_pan_display - Pans the display.
 *	@var: frame buffer variable screen structure
 *	@info: frame buffer structure that represents a single frame buffer
 *
 ...
 */
static int
omapfb_pan_display(struct fb_var_screeninfo *var, struct fb_info *info)
{
	struct omapfb_info *oinfo = (struct omapfb_info *) info->par;
	int ret;
...
/**
 *	  omapfb_blank - NOT a required function. Blanks the display.
 *
 ...
 */
static int
omapfb_blank(int blank_mode, struct fb_info *info)
{
	struct omapfb_info *oinfo = (struct omapfb_info *) info->par;
	...
	if ((blank_mode) && (!oinfo->blanked)) {
		/* Check the Vid pipeline, if it's enabled then don't allow
		 * fb to timeout.
		 */
		if (omap_disp_get_vidn_status(OMAP_VIDEO1) |
			omap_disp_get_vidn_status(OMAP_VIDEO2)) {
				return -EBUSY;
		}

		/* blank--disable display controller */
		/* wait until Vsync */
		if (omapfb_wait_for_vsync(oinfo) != 0)
			printk("omapfb_blank: vsyn wait failed \n");
		...
		omap_disp_get_dss();
#ifdef CONFIG_OMAP_USE_DSI_PLL
		omap3_disp_use_dsi_pll ();
#endif
		omap_disp_enable_layer(OMAP_GRAPHICS);
		omap_disp_enable_layer(fb_out_layer);
		omap_disp_enable_output_dev(OMAP_OUTPUT_LCD);
		omap_disp_enable_output_dev(OMAP_OUTPUT_TV);
		oinfo->blanked = 0;
		udelay(20);
		enable_backlight();
		/* omap_disp_disable function disables tv clock it has
		 * to be enabled */
		omap_disp_power_on_tv();
	}
...
/**
 *	omapfb_sync - NOT a required function. Normally the accel engine
 ...
 */
static int
omapfb_sync(struct fb_info *info)
{
	return 0;
}

/**
 *	omapfb_set_mirroring - enables and disables mirroring
 */
static int
omapfb_set_mirroring (struct fb_info *info, int mirroring)
{
	int ret = 0;
	struct omapfb_info *oinfo = (struct omapfb_info *) info->par;

	omapfb_mirroring = mirroring;
	ret = omapfb_set_dma_params(&info->var, &info->fix, (oinfo->rotation_support ? info->var.rotate : -1), omapfb_mirroring);
	if (!ret)
	{
		if (fb_out_layer == OMAP_GRAPHICS)
			omap_disp_start_gfxlayer();
...
/**
 *	omapfb_ioctl - handler for private ioctls.
 */
static int
omapfb_ioctl(struct fb_info *info, unsigned int cmd, unsigned long arg)

{
	...
	omapfb_suspend_lockout(&fb_suspend_data);
	...
	case FBIO_WAITFORVSYNC:
		 ...
		 */
		ret =  omapfb_wait_for_vsync(oinfo);
		omap_disp_put_dss();
		return ret;
	case FBIO_MIRROR:
		...
		if (!copy_from_user(&mirroring, argp, sizeof(mirroring))) {
			ret = omapfb_set_mirroring (info, mirroring);
			omap_disp_put_dss();
			return ret;
		}
...
/* PM suspend */
static int omapfb_suspend(struct platform_device *dev, pm_message_t state)
{
	struct omapfb_info *oinfo = saved_oinfo;
...
/* PM resume */
static int omapfb_resume(struct platform_device *dev)
{
	struct omapfb_info *oinfo = saved_oinfo;
...
static struct constraints omapfb_constraints = {
	.count = 2,
	.param = {
 ...
 */
static int
omapfb_scale(struct notifier_block *op, unsigned long level, void *ptr)
{
	struct omapfb_info *oinfo = saved_oinfo;
	struct fb_info *info;
	u32 clkdiv, pixclock;
	...
	case SCALE_POSTCHANGE:
		/*
		 * The Core frequency might have dropped resulting in DSS
		 * fclk drop But we would like to continue with the same
		 * DSS fclk
		 */
		...
		/* set the PCD divisor according to the new gfx_clock_period.
		   ...
		   dss1_clk is in kHz */
		oinfo->gfx_clk_period = omapfb_gfx_clk_period();
		...
		*/
		if (pixclock < omap_display_pixclock_min(output_dev))
			clkdiv = clkdiv + 1;
...
static struct notifier_block omapfb_pre_scale = {
	.notifier_call = omapfb_scale,
};

static struct notifier_block omapfb_post_scale = {
	.notifier_call = omapfb_scale,
};
...
static int
omapfb_probe (struct platform_device *dev)
{
	return 0;
}

static void
omapfb_release (struct device *dev)
{
	return;
}
...
static struct platform_device omapfb_device = {
	...
	.dev = {
#ifdef CONFIG_DPM
		.constraints    = &omapfb_constraints,
#endif
		.release = omapfb_release,
	},
};
...
#ifndef MODULE
int omapfb_setup(char *options)
{
	char *this_opt;

	printk(KERN_INFO "omapfb: Options \"%s\"\n", options);
	...
	while((this_opt = strsep(&options, ",")) != NULL) {
		...
		if (!strncmp(this_opt, "rotation=", 9)) {
			...
			printk(KERN_INFO "omapfb: Rotation %s\n",
				(omapfb_rotation == -1) ?
				"none (supported: \"rotation=[0|90|180|270]\")":
				this_opt);
		} else
			printk(KERN_INFO "omapfb: Invalid parameter \"%s\" "
...
#ifdef DEBUG_VRFB
void test_vrfb_line(struct omapfb_info *oinfo,
...
void test_vrfb_space(struct omapfb_info *oinfo,
...
write_fb_out(const char *buffer, size_t count)
{
	...
	if(out_layer != fb_out_layer){
		...
		if(omap_disp_request_layer(out_layer)){
			omap_disp_start_gfxlayer();
			fb_out_layer = out_layer;
			omapfb_set_output_layer(out_layer);
		}
	}
...
int omapfb_store_timing_info(u32 clk_div, u32 hbp, u32 hfp, u32 hsw,
                                        u32 vbp, u32 vfp, u32 vsw)
{
	struct fb_info *info=&(saved_oinfo->info);

	info->var.left_margin = hbp+1;
	info->var.right_margin = hfp+1;
	info->var.upper_margin = vbp;
	info->var.lower_margin =vfp;
	info->var.hsync_len = hsw+1;
	info->var.vsync_len = vsw+1;

	saved_oinfo->gfx_clk_period = omapfb_gfx_clk_period();
	info->var.pixclock = saved_oinfo->gfx_clk_period * clk_div;

	return 0;
}
EXPORT_SYMBOL(omapfb_store_timing_info);
...
int __init omapfb_init(void)
{
...
#ifdef CONFIG_DPM
	dpm_register_scale(&omapfb_pre_scale,SCALE_PRECHANGE);
	dpm_register_scale(&omapfb_post_scale,SCALE_POSTCHANGE);
#endif
...
#ifndef MODULE
	{
		...
		omapfb_setup(option);     // parse options
	}
#endif

	oinfo = kmalloc(sizeof(struct omapfb_info), GFP_KERNEL);
	if (!oinfo) {
		printk(KERN_ERR FB_NAME ": could not allocate memory\n");
		goto unregister_device;
	}
	memset(oinfo, 0, sizeof(struct omapfb_info));
	saved_oinfo = oinfo;

	oinfo->rotation_support = (omapfb_rotation >= 0) ? 1 : 0;

	/* set the period (in picoseconds) of the graphics timebase */
	oinfo->gfx_clk_period = omapfb_gfx_clk_period();

	/* Set base addrs */
	oinfo->fb_base_phys = omapfb_fb_base();

	oinfo->fb_size = omapfb_fb_size(oinfo->rotation_support);

	if (oinfo->rotation_support) {
		oinfo->vrfb_size = omapfb_vrfb_size();
		oinfo->rotation_deg = omapfb_rotation;
	}
	...
	if (!oinfo->alloc_fb_mem) {
		...
		oinfo->fb_base = (unsigned long)__get_free_pages(GFP_KERNEL | GFP_DMA,
						get_order(PAGE_ALIGN(oinfo->fb_size)));
		if (!oinfo->fb_base) {
			printk(KERN_ERR FB_NAME ": cannot allocate "
				"framebuffer\n");
			goto unmap_mmio;
		}
		oinfo->fb_base_phys = (dma_addr_t)virt_to_phys((void*)oinfo->fb_base);
	}
	...
	memset(oinfo->palette, 0, sizeof(u32)*256);
        ...
        if(!omap_disp_request_layer(OMAP_GRAPHICS)) {
	 ...
	 * via an ioctl.
	 */
	memcpy(&info->var, omapfb_default_var(), sizeof(info->var));
	...
	if (omapfb_check_var(&info->var, info)) {
		printk(KERN_ERR FB_NAME ": invalid default video mode\n");
		goto dealloc_cmap;
	}

	oinfo->timeout = HZ/10;  /*HZ/5;*/

	/* initialize the vsync wait queue */
	init_waitqueue_head(&oinfo->vsync_wait);

	/* install our interrupt service routine */
	if (omap_disp_register_isr(omapfb_isr, oinfo,0)) {
		printk(KERN_ERR FB_NAME
			": could not install interrupt service routine\n");
		goto dealloc_cmap;
	};
#ifdef DEBUG
	omap_disp_register_isr(omapfb_synclost_isr, oinfo,
	...
	omap_disp_save_initstate(OMAP_GRAPHICS);
	DBGLEAVE;
	return 0;
...
free_fb:
	if (!oinfo->alloc_fb_mem)
		...
		free_pages((unsigned long)oinfo->fb_base,
					get_order(PAGE_ALIGN(oinfo->fb_size)));
unmap_mmio:
...
unregister_device:
#ifdef CONFIG_DPM
	dpm_unregister_scale(&omapfb_pre_scale,SCALE_PRECHANGE);
	dpm_unregister_scale(&omapfb_post_scale,SCALE_POSTCHANGE);
#endif
