...
static int __devinit cpsw_probe(struct platform_device *pdev)
{
	...
	if (!request_mem_region(priv->cpsw_res->start, resource_size(priv->cpsw_res),
				ndev->name)) {
	...
	if (!request_mem_region(priv->cpsw_ss_res->start, resource_size(priv->cpsw_ss_res),
				ndev->name)) {
		dev_err(priv->dev, "failed request i/o region\n");
		ret = -ENXIO;
		goto clean_clk_ret;
	...
	regs = ioremap(priv->cpsw_ss_res->start, resource_size(priv->cpsw_ss_res));
	if (!regs) {
		dev_err(priv->dev, "unable to map i/o region\n");
	...
	ale_params.ale_regs		= (void*)((u32)priv->regs) + ((u32)data->ale_reg_ofs);
...
static int __devexit cpsw_remove(struct platform_device *pdev)
{
	...
	cpdma_ctlr_destroy(priv->dma);
	iounmap(priv->regs);
	release_mem_region(priv->cpsw_res->start, resource_size(priv->cpsw_res));
	release_mem_region(priv->cpsw_ss_res->start, resource_size(priv->cpsw_ss_res));
	clk_put(priv->clk);
	kfree(priv->slaves);
	free_netdev(ndev);
