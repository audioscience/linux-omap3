...
const struct usb_cppi41_info usb_cppi41_info[MAX_MUSB_INSTANCE] = {
	{
		.dma_block	= 0,
...
int __init cppi41_init(struct musb *musb)
{
	...
	u32 nIrq = TI81XX_IRQ_USBSS;

	if (cppi41_init_done)
		return 0;

	cppi_info = (struct usb_cppi41_info *)&usb_cppi41_info[musb->id];
...
static void ti81xx_set_vbus(struct musb *musb, int is_on)
{
...
int ti81xx_musb_init(struct musb *musb, void *board_data)
{
	void __iomem *reg_base = musb->ctrl_base;
	u32 rev;
	u8 mode;
	...
	musb->board_set_vbus = ti81xx_set_vbus;
...
static struct musb_platform_ops ti81xx_musb_ops = {
	...
	.read_word		= ti81xx_musb_readw,
	.read_byte		= ti81xx_musb_readb,

	.write_long		= generic_musb_writel,
	.write_word		= ti81xx_musb_writew,
	.write_byte		= ti81xx_musb_writeb,
};

static int __init ti81xx_musb_probe(struct platform_device *pdev)
{
	struct ti81xx_musb_glue          *ti81xx;
	struct platform_device          *musb;
	struct musb_hdrc_platform_data  *pdata = pdev->dev.platform_data;
	int                             ret;
	struct clk *otg_fck = NULL;

	ti81xx = kzalloc(sizeof(*ti81xx), GFP_KERNEL);
	if (!ti81xx) {
	...
	if (cpu_is_ti816x())
		ti81xx->clk = clk_get(&pdev->dev, "usbotg_ick");
	else
		ti81xx->clk = clk_get(&pdev->dev, "usb_ick");

	if (IS_ERR(ti81xx->clk)) {
		dev_err(&pdev->dev, "unable to get clock\n");
		ret = PTR_ERR(ti81xx->clk);
		goto err0;
	}
...
static int __exit ti81xx_musb_remove(struct platform_device *pdev)
{
...
static struct platform_driver ti81xx_musb_driver = {
	.remove         = __exit_p(ti81xx_musb_remove),
	.driver         = {
		.name   = "musb-ti81xx",
	},
};
...
{
	return platform_driver_probe(&ti81xx_musb_driver, ti81xx_musb_probe);
}
