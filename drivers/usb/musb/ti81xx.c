...
static void __devinit usbotg_ss_init(struct musb *musb)
{
	if (!usbss_init_done) {
...
static irqreturn_t ti81xx_interrupt(int irq, void *hci)
{
	...
	if ((usbintr & MUSB_INTR_BABBLE) && is_host_enabled(musb)) {
		ERR("CAUTION:HW Issue: Babble on musb%d\n", musb->id);
		ERR("Some EVM need long hw reset to make usb functional !!\n");
	}

	is_babble = is_host_capable() && (musb->int_usb & MUSB_INTR_BABBLE);
	if (is_babble) {
		ERR("Babble: Issue soft musb DISCONNECT");
