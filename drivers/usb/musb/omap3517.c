...
static inline void phy_on(void)
{
	 ...
	 */
	cfgchip0 = __raw_readl(OMAP2_IO_ADDRESS(OMAP3517_CONF0));

	/* AVV: can be removed later */
	cfgchip0 &= ~(0x0000FFFF);
	__raw_writel(cfgchip0, OMAP2_IO_ADDRESS(OMAP3517_CONF0));
	cfgchip0 = __raw_readl(OMAP2_IO_ADDRESS(OMAP3517_CONF0));

	/* Check whether USB0 PHY is already powered on */
	/* if (cfgchip0 & CONF0_PHY_PLLON)
		return;
	*/
	...
	while (!(__raw_readl(OMAP2_IO_ADDRESS(OMAP3517_CONF0))
			& CONF0_PHYCLKGD))
		cpu_relax();
}
...
static inline void phy_off(void)
{
	 ...
	 */
	if ((cfgchip0 & CONF0_USB1SUSPENDM) &&
		!(cfgchip0 & CONF0_USB1PHYCLKMUX)) {
		printk(KERN_WARNING
...
static irqreturn_t omap3517_interrupt(int irq, void *hci)
{
 ...
 eoi:
	...
	if (ret == IRQ_HANDLED || epintr || usbintr) {
		...
		lvl_intr = __raw_readl(
			OMAP2_IO_ADDRESS(OMAP3517_LVL_INTR_CLEAR));
		lvl_intr |= (1 << 4);
		__raw_writel(lvl_intr,
			OMAP2_IO_ADDRESS(OMAP3517_LVL_INTR_CLEAR));
		/* write EOI */
		musb_writel(reg_base, USB_END_OF_INTR_REG, 0);
	}
...
int __init musb_platform_init(struct musb *musb)
{
...
#else
	/* global reset */
	sw_reset = __raw_readl(OMAP2_IO_ADDRESS(OMAP3517_IP_SW_RESET));

	sw_reset |= (1 << 0);
	__raw_writel(sw_reset, OMAP2_IO_ADDRESS(OMAP3517_IP_SW_RESET));
	...
	printk("OMAP3517 OTG revision %08x, PHY %03x, control %02x\n",
		 rev, __raw_readl(OMAP2_IO_ADDRESS(OMAP3517_CONF0)),
	...
	/* clear level interrupt */
	lvl_intr = __raw_readl(OMAP2_IO_ADDRESS(OMAP3517_LVL_INTR_CLEAR));
	lvl_intr |= (1 << 4);
	__raw_writel(lvl_intr, OMAP2_IO_ADDRESS(OMAP3517_LVL_INTR_CLEAR));
