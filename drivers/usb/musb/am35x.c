...
u16 am35x_musb_readw(const void __iomem *addr, unsigned offset)
{
	...
	if (addr == gb_musb->mregs) {

		switch (offset) {
		case MUSB_INTRTXE:
			if (gb_musb->read_mask & AM35X_READ_ISSUE_INTRTXE)
				return gb_musb->intrtxe;
		case MUSB_INTRRXE:
			if (gb_musb->read_mask & AM35X_READ_ISSUE_INTRRXE)
				return gb_musb->intrrxe;
		default:
			break;
		}
	}
...
void am35x_musb_writew(void __iomem *addr, unsigned offset, u16 data)
{
	if (addr == gb_musb->mregs) {
		...
		switch (offset) {
		case MUSB_INTRTXE:
			gb_musb->read_mask |= AM35X_READ_ISSUE_INTRTXE;
			gb_musb->intrtxe = data;
			break;
		case MUSB_INTRRXE:
			gb_musb->read_mask |= AM35X_READ_ISSUE_INTRRXE;
			gb_musb->intrrxe = data;
		default:
			break;
		}
	}
...
u8 am35x_musb_readb(const void __iomem *addr, unsigned offset)
{
	...
	if (addr == gb_musb->mregs) {
		...
		switch (offset) {
		case MUSB_FADDR:
			if (gb_musb->read_mask & AM35X_READ_ISSUE_FADDR)
				return gb_musb->faddr;
		case MUSB_POWER:
			if (gb_musb->read_mask & AM35X_READ_ISSUE_POWER) {
				return gb_musb->power;
			} else {
				...
				gb_musb->power = val;
				gb_musb->read_mask |= AM35X_READ_ISSUE_POWER;
				return val;
			}
		case MUSB_INTRUSBE:
			if (gb_musb->read_mask & AM35X_READ_ISSUE_INTRUSBE)
				return gb_musb->intrusbe;
		default:
			break;
		}
	}
...
void am35x_musb_writeb(void __iomem *addr, unsigned offset, u8 data)
{
	if (addr == gb_musb->mregs) {
		...
		switch (offset) {
		case MUSB_FADDR:
			gb_musb->read_mask |= AM35X_READ_ISSUE_FADDR;
			gb_musb->faddr = data;
			break;
		case MUSB_POWER:
			gb_musb->read_mask |= AM35X_READ_ISSUE_POWER;
			...
			break;
		case MUSB_INTRUSBE:
			gb_musb->read_mask |= AM35X_READ_ISSUE_INTRUSBE;
			gb_musb->intrusbe = data;
		default:
			break;
		}
	}
...
static int __init am35x_musb_probe(struct platform_device *pdev)
{
	...
	am35x->fck = clk_get(&pdev->dev, "fck");
	if (IS_ERR(am35x->fck)) {
		dev_err(&pdev->dev, "unable to get fclock\n");
		ret = PTR_ERR(am35x->fck);
		goto err1;
	}
...
subsys_initcall(am35x_glue_init);
