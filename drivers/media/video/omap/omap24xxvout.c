		       ...
		       unsigned int cmd, void *arg)
{
...
#else
			((struct videobuf_dmabuf *) (q->bufs[i]->priv))->vmalloc = (void *) (virt_addr);
			((struct videobuf_dmabuf *) (q->bufs[i]->priv))->bus_addr = phy_addr;
#endif
...
#else
			vout->buf_virt_addr[i] = (unsigned long) ((struct videobuf_dmabuf *) (q->bufs[i]->priv))->vmalloc;
			vout->buf_phy_addr[i] = ((struct videobuf_dmabuf *) (q->bufs[i]->priv))->bus_addr;
#endif
	...
	case VIDIOC_QBUF:
	{
		...
		if (vout->rotation >= 0){
			...
			omap_set_dma_src_params (vout->vrfb_dma_tx.dma_ch,
				0,	// src_port required only for OMAP1
				OMAP_DMA_AMODE_POST_INC,((struct videobuf_dmabuf *)(q->bufs[buffer->index]->priv))->bus_addr,
				src_element_index, src_frame_index);
#endif			
...
#else
				omap2_disp_start_vlayer (vout->vid, &vout->pix, &vout->crop, &vout->win,
				((struct videobuf_dmabuf*)(q->bufs[buffer->index]->priv))->bus_addr,vout->rotation, vout->mirror);
#endif	
		}
...
#else
					omap2_disp_start_vlayer (dest-> vid, &dest->pix, &dest-> crop, &vout->win,
					((struct videobuf_dmabuf *)(q->bufs[buffer->index]->priv))->bus_addr, dest->rotation,
					dest->mirror);
#endif	
...
#else
	pos = ((struct videobuf_dmabuf*)(q->bufs[i]->priv))->vmalloc;
#endif
...
static void dummy_vbq_queue(struct videobuf_queue *q, struct videobuf_buffer *vb)
{
}
static int
omap24xxvout_open (struct inode *inode, struct file *file)
{
...
#else
	videobuf_queue_sg_init (q, &dummy_vbq_ops, NULL, &vout->vbq_lock,
		fh->type, V4L2_FIELD_NONE, sizeof (struct videobuf_buffer), fh);
#endif
	...
	for (i = 0; i < vout->buffer_allocated; i++) {
		...
		((struct videobuf_dmabuf*)(q->bufs[i]->priv))->vmalloc = (void *) vout->buf_virt_addr[i];
		((struct videobuf_dmabuf*)(q->bufs[i]->priv))->bus_addr = vout->buf_phy_addr[i];
#endif	
		q->bufs[i]->state = VIDEOBUF_PREPARED;
	}
