...
struct ccdc_config {
	...
	void *__iomem ccdc_intr_clr_addr;
};
...
static int vpfe_initialize_device(struct vpfe_device *vpfe_dev)
{
	...
	/*
	 * OMAP3517: Clear the VDINT0/VDINT1/VDINT2 interrupts
	 * TODO: Need to cross check whether we really need this here.
	 */
	ccdc_dev->hw_ops.clr_intr(ccdc_cfg->ccdc_intr_clr_addr, 0);
	ccdc_dev->hw_ops.clr_intr(ccdc_cfg->ccdc_intr_clr_addr, 1);
	ccdc_dev->hw_ops.clr_intr(ccdc_cfg->ccdc_intr_clr_addr, 2);
...
static irqreturn_t vpfe_isr(int irq, void *dev_id)
{
	...
	if (fid == vpfe_dev->field_id) {
		...
		if (fid == 0) {
			...
			ccdc_dev->hw_ops.clr_intr(ccdc_cfg->ccdc_intr_clr_addr,
							0);
			return IRQ_HANDLED;
		}
		/*
...
static __init int vpfe_probe(struct platform_device *pdev)
{
	...
	if (!request_mem_region(res1->start, 0x4,
	...
	/*
	 * TODO: need to pass this parameter from platform data.
	 * For DM64x - 1
	 * For OMAP3517 - 3
	 */
