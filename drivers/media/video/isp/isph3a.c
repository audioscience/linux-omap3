...
static int isph3a_aewb_set_params(struct isph3a_aewb_config *user_cfg)
{
	...
	if (unlikely((user_cfg->ver_win_count < 1) ||
		...
		return -EINVAL;
	} else if (aewb_config_local.ver_win_count != user_cfg->ver_win_count) {
	...
	if (unlikely((user_cfg->hor_win_count < 1) ||
		...
		return -EINVAL;
	} else if (aewb_config_local.hor_win_count != user_cfg->hor_win_count) {
	...
	if (unlikely(user_cfg->ver_win_start > MAX_WINSTART)) {
		...
		return -EINVAL;
	} else if (aewb_config_local.ver_win_start != user_cfg->ver_win_start) {
	...
	if (unlikely(user_cfg->hor_win_start > MAX_WINSTART)) {
		...
		return -EINVAL;
	} else if (aewb_config_local.hor_win_start != user_cfg->hor_win_start) {
