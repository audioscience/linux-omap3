...
struct spi_board_info __initdata asi1230_spi_slave_info[] = {
	...
	{
	 .modalias = "spidev",		/* 256x64 OLED (ASI2600 only) */
	 .irq = -1,
	 .max_speed_hz = 48000000,
	 .bus_num = 2,
	 .chip_select = 0,
...
static int asi1230_vsc_phy_fixup(struct phy_device *phydev)
{
	...
	printk(KERN_INFO "asi1230_vsc_phy_fixup() called.\n");
...
static int asi1230_dp_phy_fixup(struct phy_device *phydev)
{
	...
	printk(KERN_INFO "asi1230_dp_phy_fixup() called.\n");
...
#define J2_9_GPIO 5
#define CAP_TOUCH_INT_GPIO 61 /* GP1[29] */
...
static struct gpio_keys_button asi1230_gpio_buttons[] = {
	...
	{
	 .code = KEY_COFFEE,
	 .gpio = CAP_TOUCH_INT_GPIO,
	 .desc = "cap1188_interrupt",
	 .type = EV_KEY,
	 .active_low = true,
	 .wakeup = 1,
	 },
