 ...
 */
static void
omap2_disp_set_resize(int v, const short int vc[3][8], const short int hc[5][8])
{
	int i;
	unsigned long reg;
			...
			int rotation_deg, int mirroring)
{
	...
	switch (rotation_deg) {
	case 90: 
		offset = (get_vrfb_offset(pix->width,ps,SIDE_H) - (pix->width/vr_ps)) * ps * line_length;
		temp_ps = ps/vr_ps;
		if (mirroring == 0) {
	...
	case 180:
		offset = (get_vrfb_offset(pix->height,ps,SIDE_H) - pix->height) * ps * line_length +
